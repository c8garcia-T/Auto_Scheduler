...
def test_classroom():
    ...
    merged_combinations = [(i, j) for i in test_class_numbers for j in test_class_available_seats]
    for class_number, available_seats in merged_combinations:
        if not (MIN_ROOM_NUM <= class_number <= MAX_ROOM_NUM) or not (MIN_SEATS <= available_seats <= MAX_SEATS):
